...
func GetPostWithPagination(page,pagesize int)([]model.Post,int64,error){
	var total int64
	var post []model.Post
	offset:=pagesize*(page-1)
	err:=database.DB.Model(&model.Post{}).Count(&total).Error
	if err!=nil{
		return nil,0,err
	}
	result:=database.DB.Order("time DESC").Offset(offset).Limit(pagesize).Find(&post)
	if result.Error!=nil{
...
func CheckUserID (userid string)(error){
	result:=database.DB.Where("id=?",userid).First(&model.User{})
	return result.Error
}
